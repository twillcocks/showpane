...
This skill deactivates a client portal by setting `isActive` to `false` in the database. It is a soft delete -- the portal page files remain in the git repository and the database record is preserved, but the portal becomes inaccessible to clients. Existing authenticated sessions and share links will expire naturally within their normal TTL (sessions up to 7 days, share links up to 24 hours).
...
════════════════════════════════════════
  ...
  Share links affected: 1 (will expire within 24 hours)
...
- **Portal with active share link**: The share link will continue to work until it expires (up to 24 hours). If immediate revocation is needed, rotate the credentials first with `/portal credentials <slug>` (which bumps the credential version and invalidates all tokens), then deactivate.
...
- **Deactivation** (this skill): Removes the portal entirely from client view. New visitors see "not found". Existing sessions expire naturally. Use when the client relationship has ended or the portal is no longer needed.
