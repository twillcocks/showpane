...
elif [ ! -f "$APP_PATH/src/generated/prisma/client.ts" ]; then
...
1. **Direct URL**: Navigate to `http://localhost:3000/client/<slug>` to test the portal route. If no valid client auth cookie exists, it redirects to the login page with `?portal=<slug>`.
